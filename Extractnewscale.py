...
n=input('The number of layers:')

o1=np.linspace(1,len(old_frame)-1,2**(n+1)+1)  ###number of 'particles' vertically equal-spaced
o2=np.linspace(1,len(old_frame[0])-1,2**(n+1)+1)  ###number of 'particles' horizontally equal-spaced
        ...
        if index == 5:
            ...
            for column in range(4):
                for row in range(4):
                    ...
                    if Nmframe == 1:
                        q0.append([int(o2[row]),int(o1[column]),average])
                        r0.append([int(o2[row]),int(o1[column]),sd])
                    else:
                        for q in q0:
			...
			for n0 in range(2,n+1):
				for column in range(2**n0,2**(n0+1),2**(n0-1)):
					for row in range(0,2**(n0+1),2**(n0-1)):
						t0=[]
						for i in range(int(o1[column]),int(o1[column+2**(n0-1)])):
							line = flow[int(i)]
							for j in line[int(round(o2[row])):int(round(o2[row+2**(n0-1)]))]:
								t0.append(j[1])
						average = np.mean(t0)
						sd = np.std(t0)
				...
				for column in range(0,2**n0,2**(n0-1)):
					for row in range(2**n0,2**(n0+1),2**(n0-1)):
						t0=[]
						for i in range(int(o1[column]),int(o1[column+2])):
							line = flow[int(i)]
							for j in line[int(round(o2[row])):int(round(o2[row+2]))]:
								t0.append(j[1])
						average = np.mean(t0)
						sd = np.std(t0)
